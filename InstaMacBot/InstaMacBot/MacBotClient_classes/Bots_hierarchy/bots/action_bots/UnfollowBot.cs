...
namespace InstaMacBot.classes
{
    ...
    public class UnfollowBot : SSSBot
    {
        ...
        public UnfollowBot(UserApi Utente, string path_bot_save_info, BotConsole tx_console = null, int stop_fails_search_user = 10, int stop_fails_unfollow = 5, int delay = 60, Action actions = null) : base(tx_console)
        {
        ...
        private async void bot_procedure()
        {
            ...
            while (followed_list.Count > 0 && status)
            {
                ...
                followed_list.RemoveAt(0);
                 ...
                 if (follow_error >= stop_fails_unfollow)
                 {
                    stop(true);
                    return;
                 }
